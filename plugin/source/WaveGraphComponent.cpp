...
WaveGraph::WaveGraph(GraphParamSet* params, RingBuffer<float>* rBuffer)
  ...
  for (int i = 0; i < RING_BUFFER_READ_SIZE; ++i) {
    visualizationBuffer[i] =
        0.0f;  //! fill this array with zeroes to avoid weird graphs on startup
  }
}
...
void WaveGraph::paint(juce::Graphics&) {}
...
void WaveGraph::renderOpenGL() {
  ...
  if (uniforms->audioSampleData != nullptr) {
    ...
    juce::FloatVectorOperations::clear(visualizationBuffer,
                                       RING_BUFFER_READ_SIZE);
    // fill the visualization buffer with the appropriate samples for the
    // fundamental
    ...
    if ((fallingZeroCross + (int)(sampleIncrement * RING_BUFFER_READ_SIZE)) <
        readBuffer.getNumSamples()) {
      for (int i = 0; i < RING_BUFFER_READ_SIZE; ++i) {
        auto lSample = readBuffer.getSample(
            0, fallingZeroCross + (int)(sampleIncrement * i));
        auto rSample = readBuffer.getSample(
            1, fallingZeroCross + (int)(sampleIncrement * i));
...
void WaveGraph::createShaders() {
  ...
  auto shaderProgramAttempt =
      std::make_unique<juce::OpenGLShaderProgram>(openGLContext);

  // Sets up pipeline of shaders and compiles the program
  if (shaderProgramAttempt->addFragmentShader(
          {ShaderData::BasicFragment_glsl}) &&
      shaderProgramAttempt->addVertexShader({ShaderData::BasicVertex_glsl}) &&
